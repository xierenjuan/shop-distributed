...
public class OrderNoAutoUtil {
	// 使用单例模式，不允许直接创建实例
    private OrderNoAutoUtil() {}
    // 创建一个空实例对象，类需要用的时候才赋值
    private static OrderNoAutoUtil instance = null;
    // 单例模式--懒汉模式
    public static synchronized OrderNoAutoUtil getInstance() {
        if (instance == null) {
        	instance = new OrderNoAutoUtil();
        }
        return instance;
    }
    // 全局自增数
